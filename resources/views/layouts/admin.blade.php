...
<div class="flex min-h-screen">
    ...
    <!-- ================= MOBILE HEADER ================= -->
    <div class="md:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b flex items-center justify-between px-4 py-3">
    ...
    <!-- ================= SIDEBAR ================= -->
    <aside id="sidebar"
           class="w-64 bg-white border-r flex-col fixed inset-y-0 left-0 z-50
            ...
            <div x-show="open" class="ml-8 mt-1 space-y-1">
                @foreach($m['children'] as $child)
                    <a href="{{ route($child[1]) }}"
                       class="block px-4 py-2 text-sm rounded-lg hover:bg-indigo-50">
                        {{ $child[0] }}
                    </a>
                @endforeach
            </div>
...
@endforeach
        ...
        <div class="px-5 py-4 flex items-center gap-3 border-t flex-shrink-0">
            <div class="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center font-semibold text-indigo-700">
                AU
            </div>
            <div>
