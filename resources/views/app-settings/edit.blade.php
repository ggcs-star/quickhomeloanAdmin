    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            <div class="flex items-center gap-3 mb-2">
                ...
                <h1 class="text-2xl font-bold text-gray-900">Edit App Settings</h1>
            </div>
            <p class="text-sm text-gray-500">Update your application configuration</p>
        </div>

        <!-- Form Card -->
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('app-settings.update', $setting->_id) }}" method="POST" enctype="multipart/form-data" x-data="appSettingsForm({{ Js::from([
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-4">
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="is_active" value="1" {{ $setting->is_active ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 focus:ring-indigo-500">
                                        <span class="text-sm text-gray-700">Active</span>
                                    </label>
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="is_active" value="0" {{ !$setting->is_active ? 'checked' : '' }} class="w-4 h-4 text-gray-400 focus:ring-indigo-500">
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                ...
                                <div class="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center hover:border-indigo-300 transition cursor-pointer"
                                    ...
                                    <p class="text-sm text-gray-500">Click to change logo</p>
                                    <p class="text-xs text-gray-400 mt-1">JPG, PNG, WEBP (Max 2MB)</p>
                                </div>
                                <div class="mt-3">
                                    <img :src="appLogoPreview || existingAppLogo" class="h-20 w-auto rounded-lg border border-gray-200 p-1">
                                </div>
                                <p class="text-xs text-amber-600 mt-2">Leave empty to keep current logo</p>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="mt-3">
                                    <img :src="splashLogoPreview || existingSplashLogo" class="h-20 w-auto rounded-lg border border-gray-200 p-1">
                                </div>
                                <p class="text-xs text-amber-600 mt-2">Leave empty to keep current logo</p>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="mt-3">
                                    <img :src="headerLogoPreview || existingHeaderLogo" class="h-20 w-auto rounded-lg border border-gray-200 p-1">
                                </div>
                                <p class="text-xs text-amber-600 mt-2">Leave empty to keep current logo</p>
                            </div>
...
<script>
function appSettingsForm(existing = {}) {
    return {
        appName: existing.app_name || '',
        existingAppLogo: existing.app_logo || null,
        existingSplashLogo: existing.splash_logo || null,
        existingHeaderLogo: existing.header_logo || null,
        appLogoPreview: null,
